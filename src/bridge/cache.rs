...
pub struct Cache {
    ...
    raw: Box<crate::mutex::Mutex<crate::internal::NoPolicyCache>>,
}
...
impl Cache {
    ...
    pub fn __new__(
        ...
        let mut raw = crate::internal::NoPolicyCache::new(maxsize, capacity)?;
        if iterable.is_some() {
...
impl cache_iterator {
    ...
    pub fn __next__(
        ...
        if let Some(x) = l.next() {
            ...
            match slf.typ {
                0 => return Ok(key.key.clone_ref(py).as_ptr()),
                1 => return Ok(val.clone_ref(py).as_ptr()),
                2 => {
                    return tuple!(
                        py,
                        2,
                        0 => key.key.clone_ref(py).as_ptr(),
