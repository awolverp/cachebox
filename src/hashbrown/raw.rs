...
impl<T, A: Allocator> RawTable<T, A> {
    ...
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn remove_entry<E>(
        &mut self,
        hash: u64,
        eq: impl FnMut(&T) -> Result<bool, E>,
    ) -> Result<Option<T>, E> {
        // Avoid `Option::map` because it bloats LLVM IR.
        match self.find(hash, eq)? {
            Some(bucket) => Ok(Some(unsafe { self.remove(bucket).0 })),
            None => Ok(None),
        }
    }
    ...
    pub fn reserve(&mut self, additional: usize, hasher: impl Fn(&T) -> u64) {
        if likely(additional > self.table.growth_left) {
    ...
    #[inline]
    pub fn find_or_find_insert_index<E>(
        &mut self,
        hash: u64,
        mut eq: impl FnMut(&T) -> Result<bool, E>,
        hasher: impl Fn(&T) -> u64,
    ) -> Result<Result<Bucket<T>, usize>, E> {
        self.reserve(1, hasher);
        ...
        unsafe {
            ...
            match indexes {
                ...
                Ok(index) => Ok(Ok(self.bucket(index))),
                Err(index) => Ok(Err(index)),
            }
        }
    }
    ...
    pub fn get<E>(
        ...
        // Avoid `Option::map` because it bloats LLVM IR.
        match self.find(hash, eq)? {
            Some(bucket) => Ok(Some(unsafe { bucket.as_ref() })),
            None => Ok(None),
        }
    }
    ...
    pub fn get_mut<E>(
        ...
        // Avoid `Option::map` because it bloats LLVM IR.
        match self.find(hash, eq)? {
            Some(bucket) => Ok(Some(unsafe { bucket.as_mut() })),
            None => Ok(None),
        }
    }
    ...
    unsafe fn get_disjoint_mut_pointers<const N: usize>(
        ...
        array::from_fn(|i| {
            self.find(hashes[i], |k| Ok::<_, ()>(eq(i, k)))
                .unwrap()
    ...
    #[inline(always)]
    pub unsafe fn full_buckets_indices(&self) -> FullBucketsIndices {
...
pub fn prev_pow2(z: usize) -> usize {
...
impl RawTableInner {
    ...
    /// [`undefined behavior`]: https://doc.rust-lang.org/reference/behavior-considered-undefined.html
    #[inline]
    unsafe fn find_or_find_insert_index_inner<E>(
        &self,
        hash: u64,
        eq: &mut dyn FnMut(usize) -> Result<bool, E>,
            ...
            if let Some(insert_index) = insert_index {
                ...
                if likely(group.match_empty().any_bit_set()) {
                    ...
                    unsafe {
                        ...
                        return Ok(Err(self.fix_insert_index(insert_index)));
                    }
                }
            }
...
pub struct RawExtractIf<'a, T, A: Allocator> {
    pub iter: RawIter<T>,
...
impl<T, A: Allocator> RawExtractIf<'_, T, A> {
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn next<F>(&mut self, mut f: F) -> Option<T>
    where
        F: FnMut(&mut T) -> bool,
    {
        unsafe {
