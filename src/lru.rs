...
impl CacheImplemention for LRUCache {
    ...
    fn cache_setitem(&mut self, hash: isize, key: Py<PyAny>, value: Py<PyAny>) -> PyResult<()> {
        ...
        match write.insert(hash, base::KeyValuePair(key, value)) {
            Some(_) => lru_move_to_end!(order, hash),
            None => order.push_back(hash),
        }
...
crate::implement_default_functions!(LRUCache);
