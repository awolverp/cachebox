...
impl traits::OccupiedExt for Occupied<'_> {
    ...
    fn remove(self) -> Self::Handle {
        ...
        let item = self
            .policy
            .heap
            .remove(cursor, |x, y| x.frequency.cmp(&y.frequency));
...
impl LFUPolicy {
    ...
    #[inline]
    pub fn heap_mut(&mut self) -> &mut lazyheap::LazyHeap<FrequencyHandle> {
        &mut self.heap
    }
...
impl PolicyExt for LFUPolicy {
    ...
    fn evict(&mut self, _py: pyo3::Python, shared: &Self::Shared) -> pyo3::PyResult<Self::Handle> {
        {
            let front_cursor = self
                .heap
                .front(|x, y| x.frequency.cmp(&y.frequency))
        ...
        let handle = self
            .heap
            .pop_front(|x, y| x.frequency.cmp(&y.frequency))
            .unwrap();
    ...
    fn clone_ref(&mut self, py: pyo3::Python) -> Self {
        ...
        unsafe {
            for cursor in self.heap.iter(|x, y| x.frequency.cmp(&y.frequency)) {
